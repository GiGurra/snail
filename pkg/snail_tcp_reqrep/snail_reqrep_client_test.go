...
func TestNewClient_SendAndRespondWithInts_1s_batched_performance_multiple_goroutines(t *testing.T) {
	...
	batchers := make([]*snail_batcher.SnailBatcherNonBlockingPrototype[int32], nGoRoutines)
	lop.ForEach(lo.Range(nGoRoutines), func(i int, _ int) {
		client := clients[i]
		batchers[i] = snail_batcher.NewSnailBatcherNonBlockingPrototype[int32](
			batchSize,
			batchSize*2,
			1*time.Minute,
	...
	slog.Info("Setting up test window")
	withinTestWindow := snail_test_util.TrueForTimeout(testLength)
...
func TestNewClient_SendAndRespondWithJson_1s_batched_performance_multiple_goroutines(t *testing.T) {
	...
	batchers := make([]*snail_batcher.SnailBatcherNonBlockingPrototype[stupidJsonStruct], nGoRoutines)
	lop.ForEach(lo.Range(nGoRoutines), func(i int, _ int) {
		client := clients[i]
		batchers[i] = snail_batcher.NewSnailBatcherNonBlockingPrototype[stupidJsonStruct](
			batchSize,
			batchSize*2,
			1*time.Minute,
